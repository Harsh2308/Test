...
function Yachts(props) {
  ...
  const [sortOrder, setSortOrder] = useState(
    urlParams?.sortOrder ?? "length_low_to_high"
  );
        ...
        if (resp.data.Data !== null) {
          ...
          if (initPage === false) {
            ...
            yachtData = yachtData.filter((yacht) => {
              if (
                yacht.totalFeet >= lowerValue &&
                yacht.totalFeet <= upperValue
              ) {
                return yacht;
              }
              return false;
  ...
  const openDetailsPage = (id) => {
    ...
    let queryStringN = `checkInDate=${moment(checkInDate).format(
      "Y-MM-DD"
    )}&sortOrder=${sortOrder}&lowerValue=${lowerValue}&upperValue=${upperValue}&scrollPosition=${scrollPosition}`;
    window.history.replaceState(
      null,
      null,
      `${props.location.pathname}?${queryStringN}`
    );
  ...
  const replaceUrl = () => {
    const scrollPosition =
      ...
      (window.pageYOffset || document.documentElement.scrollTop);
    let queryString = `checkInDate=${moment(checkInDate).format(
      "Y-MM-DD"
    )}&sortOrder=${sortOrder}&lowerValue=${lowerValue}&upperValue=${upperValue}&scrollPosition=${scrollPosition}`;
    window.history.replaceState(
      null,
                            ...
                            <div className="dropdown">
                              ...
                              >
                                ...
                                <p
                                  className="dropdown-item url-cursor"
                                  onClick={() => {
                                    setSortOrder("length_high_to_low");
                                    setShowSortDropdown(!showSortDropdown);
                                  }}
                                >
                                  Length High to Low
              ...
              <div className="row justify-content-md-center">
                <div className="col-md-9 col-lg-7">
                  <div className="price-range">
                    <p>
                      <label>
                        Yacht Length ({numberFormat(lowerValue)} -{" "}
                        {numberFormat(upperValue)}ft.)
                      </label>
                    </p>
                    <Range
                      allowCross={false}
                      ...
                      defaultValue={[lowerBudget, upperBudget]}
                      onChange={(value) => {
              ...
              <div className="row">
                ...
                {yachts != null && yachts.length > 0 ? (
                  yachts.map((yacht) => (
                    <div
                      className="col-md-6"
                      key={yacht.transportationObjectID}
                    >
                        ...
                        <div className="row">
                          <div className="col-lg-6">
                            <div
                              ...
                              onClick={() =>
                                openDetailsPage(yacht.transportationObjectID)
                              }
                              style={{ height: "210px" }}
                            >
                              <img
                                src={yacht.imageFilePath}
                                alt=""
                            ...
                            >
                              ...
                              {yacht.fourHourRate != null ? (
                                <p className="daily-rate">
                                  <strong>${yacht.fourHourRate}</strong>/4-hours
                                </p>
                              ) : null}
                              {yacht.sixHourRate != null ? (
                                <p className="daily-rate">
                                  <strong>${yacht.sixHourRate}</strong>/6-hours
                                </p>
                              ) : null}
                              {yacht.eightHourRate != null ? (
                                <p className="daily-rate">
                                  <strong>${yacht.eightHourRate}</strong>
                                  /8-hours
                                </p>
                              ) : null}
                            </div>
